...
const parsePDF = async (dataBuffer) => {
  if (typeof pdf === "function") return pdf(dataBuffer);
  if (pdf?.default && typeof pdf.default === "function") return pdf.default(dataBuffer);
...
async function startServer() {
      ...
      const doc = new Document({
        sections: [{
          ...
          children: lines.map(line => {
            ...
            if (trimmed.length < 80 && trimmed === trimmed.toUpperCase() && trimmed.length > 2) {
  ...
  app.post("/api/ai/summarize", aiLimiter, async (req, res) => {
    try {
      ...
      const ai = getAI();
      const prompt = `You are a professional document summarizer. Provide a clear, concise summary with 4-6 key bullet points. Keep it under 300 words.
DOCUMENT TEXT:
  ...
  app.post("/api/ai/chat", aiLimiter, async (req, res) => {
    try {
      ...
      const systemContext = context
        ? `You are a helpful document assistant. Answer questions based ONLY on the document below.
DOCUMENT CONTENT:
